...
import { pollAndLockEscrow } from "../cashu";
    ...
    try {
      ...
      pollAndLockEscrow({
        ...
        onPaid: async (token, locktimeTs) => {
          // Publish funded event with the locked token
          try {
            const unsigned = {
              kind: 30404,
              content: "",
              tags: [
                ["e", proposal.id, "", "deal"],
                ["p", proposal.bodyPersonPubkey, "", "seller"],
                ["status", "funded"],
                ["cashu_token", token],
            ...
            setLockError("Paid but failed to publish: " + e.message);
          }
        },
        onError: (e) => setLockError("Escrow failed: " + e.message),
      });
    } catch (e) {
        ...
        <div className="deal-thread-body">
          ...
          {myRole === "advertiser" && fundingStep && lightningInvoice && (
            <div className="deal-actions">
              {!escrowLocked ? (
                <>
                  <p className="deal-action-desc">Pay this Lightning invoice to lock the sats in Cashu escrow:</p>
                  <div className="invoice-box">
                    <code className="invoice-text">{lightningInvoice.slice(0, 60)}…</code>
                    <div className="invoice-actions">
                      <button className="btn-primary" onClick={() => navigator.clipboard.writeText(lightningInvoice)}>
                        Copy Invoice
                      </button>
                      <a className="btn-primary" href={`lightning:${lightningInvoice}`}>
                        Open in Wallet
                      </a>
                    </div>
                  </div>
                  <div className="polling-status">
                    ...
                    <span>Waiting for payment… (checking every 3s)</span>
                  </div>
                  {lockError && <div className="error-msg">{lockError}</div>}
                  <p className="dim-small">After paying, tokens are locked automatically. Neither party can move them without both signatures until the 7-day timelock expires.</p>
                </>
              ) : (
                <div className="escrow-locked-banner">
                  <Shield size={16} color="#39ff14" />
                  <span>Escrow locked! ⚡ Sats are secured. Body person can now proceed.</span>
                </div>
              )}
            </div>
          )}
